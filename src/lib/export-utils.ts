...
export function generateMonthlyFeeReportData(
  classData: Array<{
    className: string
    studentCount: number
    yearlyTotals: { totalExpected: number; totalCollected: number; outstanding: number }
    collectionRate: number
    monthlyData: Array<{ month: string; totalExpected: number; totalCollected: number }>
  }>,
  year: number,
): ExportData {
  const columns: ExportColumn[] = [
    ...
    { key: "totalExpected", label: "Total Expected", width: 15, format: "currency" },
    { key: "totalCollected", label: "Total Collected", width: 15, format: "currency" },
    { key: "outstanding", label: "Outstanding", width: 15, format: "currency" },
    { key: "collectionRate", label: "Collection Rate", width: 15, format: "percent" },
  ]
...
export function generateDefaultersReportData(
  defaulters: Array<{
    student: { studentName: string; registrationNumber: string; fatherMobile?: string }
