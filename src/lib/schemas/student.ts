...
export const studentSchema = z.object({
  ...
  fatherProfession: z.string().max(100).nullish().transform(v => v ?? ""),
  bloodGroup: z.string().max(3).nullish().transform(v => v ?? ""),
  guardianName: z.string().max(100).nullish().transform(v => v ?? ""),
