...
export default function EmployeeMonthlyAttendancePage() {
  ...
  const { data: attendanceData } =
    api.attendance.getAllEmployeeAttendance.useQuery();
  ...
  const filteredAttendanceData = useMemo(() => {
    return (
      attendanceData?.filter((att) => {
        ...
        return (
          attDate.isAfter(dayjs(startOfMonth).subtract(1, "day")) &&
          attDate.isBefore(dayjs(endOfMonth).add(1, "day"))
        );
      }) ?? []
    );
  ...
  const filteredEmployees = useMemo(() => {
    const employeeList =
      selectedDesignation === "ALL" ? allEmployees : employees;
    return (
  ...
  const getDesignationBadge = (accountType: string) => {
    ...
    return (
      <Badge
        variant={colors[accountType as keyof typeof colors] ?? "outline"}
        className="text-xs"
      >
        {accountType}
      </Badge>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            View monthly attendance records for all staff members
          </p>
        </div>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center space-x-4">
            <Select
              ...
              onValueChange={(value) =>
                setSelectedDesignation(value as Designation)
              }
            >
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold">
                {designations.find((d) => d.value === selectedDesignation)
                  ?.label ?? "All Employees"}{" "}
                - {monthName} {currentYear}
              </h3>
                ...
                <TableBody>
                  {filteredEmployees.map((employee) => {
                    const presentDays = days.filter(
                      (day) =>
                        getAttendanceStatus(employee.id, day) === "PRESENT",
                    ).length;
                    const attendancePercentage = Math.round(
                      (presentDays / daysInMonth) * 100,
                    );
                    ...
                    return (
                      <TableRow key={employee.id}>
                        ...
                        <TableCell>
                          {getDesignationBadge(employee.accountType)}
                        </TableCell>
                        {days.map((day) => (
                          <TableCell key={day} className="text-center">
                            {getStatusBadge(
                              getAttendanceStatus(employee.id, day),
                            )}
                          </TableCell>
                        ))}
