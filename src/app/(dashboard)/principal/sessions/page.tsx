// File: src/app/(dashboard)/principal/sessions/page.tsx
"use client";
  ...
  ({
    ...
    const styles = {
      emerald: {
        ...
        text: "text-emerald-900 dark:text-foreground",
        subText: "text-emerald-600/70 dark:text-emerald-100/60",
        iconBox:
          "bg-emerald-100 dark:bg-emerald-500/20 ring-emerald-200 dark:ring-white/10",
        iconColor: "text-emerald-600 dark:text-foreground",
        trendBadge:
          "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/20",
      ...
      amber: {
        ...
        iconBox:
          "bg-amber-100 dark:bg-amber-500/20 ring-amber-200 dark:ring-white/10",
        iconColor: "text-amber-600 dark:text-foreground",
        trendBadge:
      ...
      blue: {
        ...
        iconBox:
          "bg-blue-100 dark:bg-blue-500/20 ring-blue-200 dark:ring-white/10",
        iconColor: "text-blue-600 dark:text-foreground",
        trendBadge:
          "bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400 border-blue-200 dark:border-blue-500/20",
        gradient: "from-blue-400 to-indigo-600",
      },
      pink: {
        border: "border-pink-200 dark:border-pink-500/10",
        bg: "bg-pink-50 dark:bg-white/5",
        text: "text-pink-900 dark:text-foreground",
        subText: "text-pink-600/70 dark:text-pink-100/60",
        iconBox:
          "bg-pink-100 dark:bg-pink-500/20 ring-pink-200 dark:ring-white/10",
        iconColor: "text-pink-600 dark:text-foreground",
        trendBadge:
          "bg-pink-100 text-pink-700 dark:bg-pink-500/20 dark:text-pink-400 border-pink-200 dark:border-pink-500/20",
    ...
    return (
      <motion.div
        ...
        className={cn(
          "group relative overflow-hidden rounded-xl border p-4 shadow-sm backdrop-blur-md transition-all hover:shadow-md",
          styles.border,
          styles.bg,
        )}
      >
        ...
        <div
          className={`absolute -right-10 -top-10 h-32 w-32 rounded-full bg-gradient-to-br ${styles.gradient} opacity-5 blur-3xl transition-opacity group-hover:opacity-10 dark:opacity-10 dark:group-hover:opacity-20`}
        />
        ...
        <div className="relative z-10 flex items-start justify-between">
          ...
          <div
            className={cn("rounded-lg p-2 shadow-sm ring-1", styles.iconBox)}
          >
        ...
        {stat.trend && !isLoading && (
          <div className="mt-3 flex items-center gap-1.5">
            <div
              className={cn(
                "flex items-center justify-center rounded-full border px-1.5 py-0.5",
                styles.trendBadge,
              )}
            >
              <TrendingUp className="h-3 w-3" />
            </div>
            <span className={cn("text-xs font-bold", styles.subText)}>
              {stat.trend} increase
            </span>
          </div>
        )}
...
export default function SessionFeePage() {
  const breadcrumbs = [
    { href: "/principal", label: "Admin" },
    { href: "/principal/sessions", label: "Sessions & Fees", current: true },
  ];
        ...
        <CardHeader className="border-b border-slate-100 bg-slate-50/50 px-4 py-3 dark:border-border dark:bg-white/5">
          <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
            ...
            <Button
              size="sm"
              variant="outline"
              className="h-8 border-emerald-200 bg-emerald-50 text-xs text-emerald-700 transition-colors hover:bg-emerald-100 hover:text-emerald-800 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-300 dark:hover:bg-emerald-500/20 dark:hover:text-emerald-200"
            >
