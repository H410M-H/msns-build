    ...
    >
      ...
      {stat.trend && !isLoading && (
        <div className="mt-3 flex items-center gap-1.5">
          <div className="flex items-center justify-center rounded-full bg-emerald-500/20 px-1.5 py-0.5 border border-emerald-500/20">
            <TrendingUp className="h-3 w-3 text-emerald-400" />
          </div>
          <span className="text-xs font-medium text-emerald-400">{stat.trend} increase</span>
        </div>
      )}
    </motion.div>
  );
...
const DashboardSkeleton = () => (
  <div className="space-y-4">
    ...
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      {Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-32 rounded-xl bg-emerald-900/20" />)}
    </div>
...
const TableSkeleton = () => (
  <div className="w-full space-y-2 p-4">
    ...
    {Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-12 w-full rounded-lg bg-white/5" />)}
  </div>
);
...
export default function SessionFeePage() {
  ...
  const stats = useMemo<StatConfig[]>(() => {
    ...
    return [
      { id: "students", title: "Total Students", value: studentData?.length ?? 0, icon: Users, gradient: "from-emerald-400 to-green-600", textColor: "text-emerald-400", trend: "12%" },
      { id: "revenue", title: "Revenue", value: totalRevenue, icon: DollarSign, gradient: "from-amber-400 to-orange-600", textColor: "text-amber-400", prefix: "Rs. ", trend: "8%" },
      { id: "classes", title: "Active Classes", value: classData?.length ?? 0, icon: School, gradient: "from-blue-400 to-indigo-600", textColor: "text-blue-400" },
      { id: "faculty", title: "Faculty", value: employeeData?.length ?? 0, icon: GraduationCap, gradient: "from-purple-400 to-pink-600", textColor: "text-pink-400" },
    ];
  }, [studentData, feeData, classData, employeeData]);

  if (error) return <div className="text-center p-8 text-red-400">Error loading data.</div>;
  ...
  return (
    ...
    <div className="w-full space-y-4">
      ...
      <div className="flex flex-col justify-between gap-2 md:flex-row md:items-end">
        ...
        {sessionData && (
          <div className="flex items-center gap-2 rounded-full border border-emerald-500/20 bg-emerald-500/5 px-3 py-1 backdrop-blur-xs">
            ...
            <span className="text-xs font-medium text-emerald-200">{sessionData.sessionName}</span>
          </div>
        )}
      </div>
      ...
      <Card className="overflow-hidden border border-emerald-500/10 bg-slate-900/40 backdrop-blur-xl shadow-2xl">
        ...
        <CardHeader className="border-b border-white/5 bg-white/5 px-4 py-3">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <CardTitle className="text-base font-bold text-white">Session Registry</CardTitle>
              </div>
            </div>
            <Button 
              size="sm"
              variant="outline" 
              className="h-8 text-xs border-emerald-500/30 bg-emerald-500/10 text-emerald-300 hover:bg-emerald-500/20 hover:text-emerald-200"
            >
