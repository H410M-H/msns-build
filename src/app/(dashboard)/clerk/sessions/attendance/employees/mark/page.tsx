...
import { CalendarDays, Search, Save, Users, Clock, CheckCircle2, XCircle, AlertCircle, Briefcase } from "lucide-react"
import { api } from "~/trpc/react"
import { toast } from "sonner"

type AttendanceStatus = "PRESENT" | "ABSENT" | "LATE" | "EXCUSED"
type Designation = "PRINCIPAL" | "ADMIN" | "HEAD" | "CLERK" | "TEACHER" | "WORKER"
...
export default function MarkEmployeeAttendancePage() {
  ...
  const handleTimeChange = (employeeId: string, field: "checkInTime" | "checkOutTime", time: string) => {
    setAttendanceData((prev) => {
      const existing = prev[employeeId]
      if (!existing) return prev // do not update if no status set yet
      return {
  ...
  const handleSubmit = () => {
    ...
    const attendanceRecords = Object.entries(attendanceData).map(([employeeId, data]) => ({
      employeeId,
      date: selectedDate,
      status: data.status,
      notes: data.notes,
  ...
  const filteredEmployees = employees?.filter(
    (employee) =>
      ...
      employee.registrationNumber.toLowerCase().includes(searchTerm.toLowerCase()),
  )
  ...
  return (
    <div className="container mx-auto p-6 space-y-6 bg-gradient-to-br from-slate-50 to-purple-50 min-h-screen">
      ...
      <Card className="border-0 shadow-lg">
        <CardHeader>
          <CardTitle className="text-xl text-slate-800">Select Department & Date</CardTitle>
          <CardDescription>Choose the designation and date to mark attendance</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <CardContent>
            <div className="space-y-4">
              {filteredEmployees?.map((employee) => {
                const currentStatus = attendanceData[employee.employeeId]?.status
                const currentNotes = attendanceData[employee.employeeId]?.notes ?? ""
                const checkInTime = attendanceData[employee.employeeId]?.checkInTime ?? ""
                  ...
                  <div key={employee.employeeId} className="p-4 border rounded-lg bg-white shadow-sm">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-500">ID: {employee.registrationNumber}</p>
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      <div className="space-y-3">
                        <div>
                          <Label className="text-sm font-medium text-slate-700 mb-2 block">Attendance Status</Label>
                          <div className="flex gap-2 flex-wrap">
                            {(["PRESENT", "ABSENT", "LATE", "EXCUSED"] as AttendanceStatus[]).map((status) => (
                              <Button
                                ...
                                size="sm"
                                onClick={() => handleStatusChange(employee.employeeId, status)}
                                className={currentStatus === status ? getStatusColor(status) : ""}
                              >
                          ...
                          <div className="grid gap-2 md:grid-cols-2">
                            <div>
                              ...
                              <Input
                                ...
                                type="time"
                                value={checkInTime}
                                onChange={(e) => handleTimeChange(employee.employeeId, "checkInTime", e.target.value)}
                              />
                            </div>
                            <div>
                              <Label
                              ...
                              <Input
                                ...
                                type="time"
                                value={checkOutTime}
                                onChange={(e) => handleTimeChange(employee.employeeId, "checkOutTime", e.target.value)}
                              />
                            </div>
                          </div>
                        )}
                      ...
                      <div>
                        ...
                        <Textarea
                          ...
                          value={currentNotes}
                          onChange={(e) => handleNotesChange(employee.employeeId, e.target.value)}
