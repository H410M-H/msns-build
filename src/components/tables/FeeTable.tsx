...
const columns: ColumnDef<FeeProps>[] = [
  ...
  },
  {
    ...
    header: "Monthly Fee",
    cell: ({ getValue }) => (
      <span className="font-semibold text-emerald-700">Rs. {(getValue() as number).toLocaleString()}</span>
    ),
  },
  {
    accessorKey: "admissionFee",
    header: "Admission",
    cell: ({ getValue }) => <span className="text-slate-600">Rs. {(getValue() as number).toLocaleString()}</span>,
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <span className="font-medium text-purple-700">Rs. {annualFee.toLocaleString()}</span>
    },
  },
  {
    ...
    cell: ({ getValue }) => {
      ...
      return <span className="text-sm text-slate-500">{format(date, "dd MMM yyyy")}</span>
    },
  },
]
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-600">Manage fee templates for different class levels</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
        {Object.entries(totalFeesByClass).map(([level, fee]) => (
          <div key={level} className="bg-white p-4 rounded-lg border shadow-sm">
            ...
            <p className="text-lg font-bold text-slate-900">Rs. {fee.toLocaleString()}</p>
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-600">
          Showing {table.getRowModel().rows.length} of {fees?.length ?? 0} entries
        </p>
