...
export const CSVUploadDialog: React.FC<CSVUploadDialogProps> = ({
  ...
  const parseCSV = (text: string): StudentCSVSchema[] => {
    ...
    for (let i = 0; i < Math.min(lines.length, 20); i++) {
      ...
      const hasStudentName = normalizedHeaders.some(
        (h) => h.includes("studentname") || h.includes("nameofstudent"),
      );
      const hasFatherName = normalizedHeaders.some((h) => h.includes("father"));
      const hasAdmission = normalizedHeaders.some((h) =>
        h.includes("admission"),
      );

      if (
        (hasStudentName && hasFatherName) ||
        (hasStudentName && hasAdmission)
      ) {
        headerRowIndex = i;
        headers = potentialHeaders.map((h) =>
    ...
    const mapHeader = (h: string): keyof StudentCSVSchema | null => {
      ...
      if (
        normalizedH.includes("studentname") ||
        normalizedH.includes("nameofstudent")
      )
        return "studentName";
      if (normalizedH === "fathersname" || normalizedH === "fathername")
        return "fatherName";
      if (normalizedH === "dateofbirth" || normalizedH === "dob")
        return "dateOfBirth";
      if (
        normalizedH.includes("dateofadmission") ||
        normalizedH.includes("admissiondate")
      )
        return "dateOfAdmission";
      if (normalizedH === "address" || normalizedH === "residence")
        return "address";
      if (normalizedH.includes("contact") || normalizedH.includes("mobile"))
        return "contactNumber";
      if (
        normalizedH.includes("occupation") ||
        normalizedH.includes("0ccupation")
      )
        return "fatherOccupation";
      if (normalizedH.includes("caste") || normalizedH.includes("tribe"))
        return "caste";
      if (normalizedH.includes("reg") || normalizedH === "registrationnumber")
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      if (parsedData.length === 0) {
        throw new Error(
          "No valid student data found. Please check your CSV headers.",
        );
      }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="border-emerald-500/20 bg-card text-foreground sm:max-w-[425px]">
        <DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      ...
                      type="file"
                      accept=".csv"
                      className="border-emerald-500/30 bg-card text-foreground file:text-emerald-400 file:hover:text-emerald-300"
                      onChange={(event) => {
            ...
            {isUploading && (
              <div className="space-y-2">
                <Progress value={undefined} className="h-1 w-full bg-muted">
                  <div className="animate-indeterminate h-full bg-emerald-500" />
                </Progress>
                <p className="animate-pulse text-center text-xs text-emerald-500">
                  Processing...
                </p>
              </div>
            )}
            <DialogFooter>
              <Button
                ...
                className="w-full bg-emerald-600 text-foreground hover:bg-emerald-500 sm:w-auto"
              >
                {isUploading ? (
                  <>
